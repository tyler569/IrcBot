    ...
    def d2bin(self, sender, *args):
        ...
        if len(args) == 1:
            try:
                ...
                self.send("d2bin required one whole number as an argument", sender)
        else:
            self.send("d2bin required one whole number as an argument", sender)
            
    def echo(self, sender, *args):
        if len(args) > 0:
            self.send(" ".join(args))
        else:
            self.send("You need to provide a message to echo", sender)
